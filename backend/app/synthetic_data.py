...
def generate_transactions(n=2000, n_accounts=500):
    ...
    for i in range(n):
        ...
        ts = start + timedelta(minutes=int(np.random.exponential(scale=60*24)))
        rows.append({"tx_id": f"TX{i}", "sender": sender,
                     "receiver": receiver, "amount": amount,
                     "timestamp": ts})
    df = pd.DataFrame(rows)
    return df

# inject a money-mule ring
def inject_money_mule(df, leader="ACC1", ring_size=8, txs_per_leader=20):
    # leader disperses funds rapidly to ring accounts
    ring = [f"ACC{2+i}" for i in range(ring_size)]
    now = df['timestamp'].max() + pd.Timedelta(hours=1)
    for i in range(txs_per_leader):
        for r in ring:
            df = df.append({
                ...
                "timestamp": now + pd.Timedelta(seconds=i*10)
            }, ignore_index=True)
    return df

if __name__ == "__main__":
    df = generate_transactions(1000, 300)
    df = inject_money_mule(df, leader="ACC1", ring_size=6, txs_per_leader=10)
    print(df.tail())
